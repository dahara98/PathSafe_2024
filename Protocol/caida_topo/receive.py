...
SIP_KEY_0 = 0x0706050403020100
SIP_KEY_1 = 0x0f0e0d0c0b0a0908
...
class SipHash:
    ...
    def set_secrets(self):
        ...
        # self.paths = {0:['s1','s2','s4','s8','s15'],1:['s1','s2','s4','s9','s8','s15'],
        # 2:['s1','s2','s4','s8','s15'],3:['s1','s2','s4','s9','s8','s15'],
        # 4:['s1','s2','s4','s8','s15'],5:['s1','s2','s4','s9','s8','s15'],
        # 6:['s2','s4','s8','s15'],7:['s2','s4','s9','s8','s15'],
        # 8:['s2','s4','s8','s15'],9:['s2','s4','s9','s8','s15'],
        # 10:['s2','s4','s8','s15'],11:['s2','s4','s9','s8','s15']
        # 12:['s3','s2','s4','s8','s15'],13:['s3','s2','s4','s9','s8','s15'],
        # 14:['s3','s2','s4','s8','s15'],15:['s3','s2','s4','s9','s8','s15'],
        # 16:['s3','s2','s4','s8','s15'],17:['s3','s2','s4','s9','s8','s15'],}
        # 18:['s4','s8','s15'],19:['s4','s9','s8','s15']}
        self.paths = {0:['s1','s2','s4','s9','s8','s15'],1:['s1','s2','s4','s8','s15']}
    ...
    def sip_init(self):
        self.hdr['sip_meta']['v_0'] = CONST_0 ^ SIP_KEY_0
        self.hdr['sip_meta']['v_1'] = CONST_1 ^ SIP_KEY_1
        self.hdr['sip_meta']['v_2'] = CONST_2 ^ SIP_KEY_0
        self.hdr['sip_meta']['v_3'] = CONST_3 ^ SIP_KEY_1

    def sip_preround_1(self):

        lower = self.hdr['sip_meta']['v_3'] & 0x00000000ffffffff
    ...
    def sip_preround_2(self):
        ...
        a = self.hdr['sip_meta']['v_3'] & 0x00000000ffffffff
        b = self.ig_md['sip_tmp']['i_0'] & 0x00000000ffffffff
        a = self.pad(a,32)
        b = self.pad(b,32)
    ...
    def sip_3_a1(self):
        ...
        keep = self.ig_md['sip_tmp']['a_1'] & 0x00000000FFFFFFFF
    ...
    def sip_3_b1(self):
        ...
        keep = self.ig_md['sip_tmp']['a_3'] & 0x00000000FFFFFFFF
        keep = self.pad(keep,32)
    ...
    def sip_4_a1(self):
        ...
        keep = self.hdr['sip_meta']['v_2'] & 0x00000000ffffffff
        keep = self.pad(keep,32)
        add = self.ig_md['sip_tmp']['a_2'] & 0x00000000ffffffff
        add = self.pad(add,32)
    ...
    def sip_postround_1(self):
        ...
        keep = self.hdr['sip_meta']['v_0'] & 0x00000000FFFFFFFF
        keep = self.pad(keep,32)
    ...
    def sip_postround_2(self):
        ...
        b = self.ig_md['sip_tmp']['i_0'] & 0x00000000FFFFFFFF
        a = self.ig_md['sip_tmp']['a_0'] & 0x00000000FFFFFFFF
        a = self.pad(a,32)
        b = self.pad(b,32)
        xor = int(a,2) ^ int(b,2)
        xor = self.pad(xor,32)
    ...
    def sip_speculate_end_1(self):
        ...
        keep = self.ig_md['sip_tmp']['hval'] & 0x00000000FFFFFFFF
        keep = self.pad(keep,32)
    ...
    def sip_speculate_end_2(self):
        ...
        v0 = self.hdr['sip_meta']['v_0'] & 0x00000000FFFFFFFF
        v1 = self.hdr['sip_meta']['v_1'] & 0x00000000FFFFFFFF
        v2 = self.hdr['sip_meta']['v_2'] & 0x00000000FFFFFFFF
        v3 = self.hdr['sip_meta']['v_3'] & 0x00000000FFFFFFFF
        add = v0 ^ v1 ^ v2 ^ v3
        add = self.pad(add,32)
    ...
    def compute(self,ttl,session_id,egress_port,hash):
    ...
    def set_paths_mac(self,ttl,session,egress_last):
        ...
        for p,path in enumerate(self.paths.values()):
            ...
            for h,hop in enumerate(path):
                ...
                new_hash = self.compute(path_ttl,session,output_port,hash)
                path_ttl = path_ttl-1
                hash = new_hash
    ...
    def handle_pkt(self,pkt,iface):
        ...
        if ProbePacket in pkt and (iface.split('-')[0] == 's15'):
            t0 = time.time()
            ...
            for k,v in paths_macs.items():
                if hash == v:
                    print('verified path',k)
                    print("Single path verification time",time.time()-t0)
                    print("Final timestamp",time.time())
    ...
    def main(self):
        ...
        iface = self.topo.get_cpu_port_intf('s15')
